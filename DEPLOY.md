...
- explicit NEON release/cutoff and bundle build date;
- site, environment, reference, authority, and index inventory;
- `manifest.json` SHA-256;
- CI run URL.

The normative gates are in [Build–Test Handoff](docs/BUILD-TEST-HANDOFF.md). Do not substitute a successful local page load for the R, byte, deployment, or public-browser receipts.
...
8. Attach screenshots, manifest hash, commit, deployment identifier, and workflow URLs to the release receipt.
...
1. resolve an explicit closed cutoff and release receipt;
2. fetch exactly 46 sites on R `4.1.1` into empty, read-only staging;
3. build twice on R `4.5.2` in separate roots;
4. compare exact derived bytes and rebuild completeness plus the runtime receipt;
...
`skip_download=true` means revalidate the committed inputs; it is not permission to reuse an unknown or partial cache. Any failed or missing site exits non-zero. Old and new bundles must never be mixed.
